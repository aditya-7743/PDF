...
export function renderThumbnailSlideHtml(q, settings, idx) {
  ...
  const isBlank = (q.layout === "blank") && !hasAnyActiveElement;
  const imgList = getQuestionImages(q);
  let bgSize = "100% 100%";
  if (settings.bgFit === "cover") bgSize = "cover";
  else if (settings.bgFit === "contain") bgSize = "contain";

  const bgImgUrl = q.bgImage || (q.settings && q.settings.bgImage) || settings.bgImage;
  const slideBgColor = (q.settings && q.settings.slideBg) || settings.slideBg || '#FFFFFF';
  ...
  if (isBlank) {
    return `
      <div class="ppt-slide-canvas-wrapper" style="${bgStyle} position:relative; width:960px; height:540px; overflow:hidden;">
        ...
        ${!imgList.length && !q.english && !bgImgUrl ? `
  ...
  return `
    <div class="ppt-slide-canvas-wrapper" style="${bgStyle} position:relative; width:960px; height:540px; overflow:hidden; display:flex; flex-direction:column;">
      ...
      <div class="slide-body-content" style="padding:${settings.questionPadding || 16}px 24px; transform:translate(${posXPercent}%, ${settings.boxPosY || 0}px); width:${widthPercent}%; flex:1; display:flex; flex-direction:column;">
        ...
        <!-- English -->
        <div style="display:${settings.showEnglish ? 'block' : 'none'}; transform:translate(${settings.engPosX || 0}px, ${settings.engPosY || 0}px); width:${settings.engWidth ? `${settings.engWidth}%` : '100%'};">
        ...
        <!-- Divider -->
        <div style="display:${settings.showDivider ? 'block' : 'none'}; width:${settings.dividerWidth ? `${settings.dividerWidth}%` : '100%'}; transform:translate(${settings.dividerPosX || 0}px, ${settings.dividerPosY || 0}px); margin:${settings.dividerSpacing || 6}px 0;">
          <div style="border-top:${settings.dividerThickness || 2}px solid ${settings.dividerColor || '#A30000'};"></div>
        </div>

        <!-- Hindi -->
        <div style="display:${settings.showHindi ? 'block' : 'none'}; transform:translate(${settings.hindiPosX || 0}px, ${settings.hindiPosY || 0}px); width:${settings.hindiWidth ? `${settings.hindiWidth}%` : '100%'};">
